...
impl Instance {
    pub fn new(config: impl ConfigLoader + 'static) -> Self {
        ...
        if let Ok(secrets_file) = std::env::var("EXTRA_SECRETS_FILE") {
            global_ctx.set_extra_secrets_file(secrets_file);
            let ctx = global_ctx.clone();
            tokio::spawn(async move {
                loop {
                    tokio::time::sleep(std::time::Duration::from_secs(30)).await;
                    ctx.load_extra_identities_from_json();
                }
            });
        }
        ...
        println!("[WHITELIST_DEBUG] Checking IP_WHITELIST_FILE env var...");
        if let Ok(whitelist_file) = std::env::var("IP_WHITELIST_FILE") {
            global_ctx.set_ip_whitelist_file(whitelist_file.clone());
            println!("[WHITELIST_DEBUG] IP_WHITELIST_FILE = {}", whitelist_file);
            if let Ok(auto_file) = std::env::var("IP_HOSTNAME_AUTO_FILE") {
                global_ctx.set_ip_hostname_auto_file(auto_file);
            } else {
            ...
            let ev_pm = peer_manager.clone();
            tokio::spawn(async move {
                println!("[WHITELIST_DEBUG] Periodic task started");
                loop {
                    tokio::time::sleep(std::time::Duration::from_secs(30)).await;
                    println!("[WHITELIST_DEBUG] Periodic task running");
                    let whitelist = ctx.load_ip_whitelist();
                    println!("[WHITELIST_DEBUG] whitelist has {} entries", whitelist.len());
                    let routes = pm.list_routes().await;
                    println!("[WHITELIST_DEBUG] routes from list_routes(): {} total", routes.len());
                    for route in &routes {
                    ...
                    for route in &routes {
                        let ip_str = route.ipv4_addr.as_ref().and_then(|ip| ip.address.as_ref()).map(|a| format!("{}", a));
                        println!("[WHITELIST_DEBUG] checking peer_id={}, ipv4={:?}", route.peer_id, ip_str);
                        if let Some(ip) = ip_str {
                            let whitelist_ip_match = whitelist.iter().any(|e| e.ip == ip);
                            if whitelist_ip_match {
                                let entry = whitelist.iter().find(|e| e.ip == ip).unwrap();
                                match &entry.hostname {
                                    Some(bound_hostname) if !bound_hostname.is_empty() => {
                                        if route.hostname != *bound_hostname {
                                    ...
                                    _ => {
                                        // hostname is None or "" — allow peer, auto-bind for both
                    ...
                    for peer_id in allowed_peers {
                        if ctx.is_peer_blocked(peer_id) {
                            ctx.unblock_peer(peer_id);
                            tracing::info!("Unblocked peer {}: IP now in whitelist", peer_id);
                        }
                    }
                }
            });
            tokio::spawn(async move {
                println!("[WHITELIST_DEBUG] Event-driven task started, subscribing to events");
                let mut rx = ev_ctx.subscribe();
                loop {
                    match rx.recv().await {
                        Ok(GlobalCtxEvent::PeerAdded(peer_id)) => {
                            tracing::info!("Event-driven: PeerAdded({})", peer_id);
                            let ctx = ev_ctx.clone();
                            let pm = ev_pm.clone();
                            tokio::spawn(async move {
                                let whitelist = ctx.load_ip_whitelist();
                                ...
                                for iteration in 0..120 {
                                    ...
                                    if iteration % 30 == 0 {
                                        tracing::info!("Event-driven: polling route info for peer {}, iteration {}", peer_id, iteration);
                                    }
                                    match pm.get_peer_map().get_route_peer_info(peer_id).await {
                                        Some(info) => {
                                            if let Some(ref ip) = info.ipv4_addr {
                                                ...
                                                if whitelist_ip_match {
                                                    let entry = whitelist.iter().find(|e| e.ip == ip_str).unwrap();
                                                    let peer_hostname = info.hostname.as_deref().unwrap_or("");
                                                    match &entry.hostname {
                                                        Some(bound_hostname) if !bound_hostname.is_empty() => {
                                                            if peer_hostname != bound_hostname.as_str() {
                                                                if !ctx.is_peer_blocked(peer_id) {
                                                                    ctx.block_peer(peer_id);
                                                                    let _ = pm.get_peer_map().close_peer(peer_id).await;
                                                                ...
                                                                if ctx.is_peer_blocked(peer_id) {
                                                                    ctx.unblock_peer(peer_id);
                                                                    tracing::info!("Event-driven: unblocked peer {}: IP {} in whitelist with matching hostname", peer_id, ip_str);
                                                            }
                                                        }
                                                        }
                                                        _ => {
                                                            ...
                                                            if let Some(ref h) = info.hostname {
                                                                ctx.bind_hostname(&ip_str, h);
                                                            }
                                                            if ctx.is_peer_blocked(peer_id) {
                                                                ctx.unblock_peer(peer_id);
                                                                tracing::info!("Event-driven: unblocked peer {}: IP {} in whitelist (auto-bound hostname)", peer_id, ip_str);
                                                            }
                                                        }
                                                    }
                                                } else {
                                                ...
                                                tracing::debug!("Event-driven: peer {} has route info but ipv4_addr is None (iteration {})", peer_id, iteration);
                                            }
                                        }
                                        None => {
                                            if iteration % 30 == 0 {
                                                tracing::info!("Event-driven: no route info yet for peer {} (iteration {})", peer_id, iteration);
                                            }
                                        }
                                    }
                                }
                                tracing::warn!("Event-driven: no route info for peer {} within 120s", peer_id);
                            });
                        }
                        Ok(_) => {}
                        Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
                            tracing::warn!("Whitelist event subscriber skipped {} events", n);
                        }
                ...
                loop {
                    ...
                    tokio::task::spawn_blocking(move || {
                        ...
                        let uri = match http_req::uri::Uri::try_from(url_clone.as_str()) {
                            Ok(u) => u,
                            Err(e) => {
                                tracing::warn!("Failed to parse admin API URL: {}", e);
                                return;
                            }
                        };
