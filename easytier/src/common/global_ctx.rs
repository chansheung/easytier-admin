...
pub struct GlobalCtx {
    ...
    extra_network_identities: Mutex<Vec<NetworkIdentity>>,

    extra_secrets_file: Mutex<Option<String>>,
...
impl GlobalCtx {
    pub fn new(config_fs: impl ConfigLoader + 'static) -> Self {
        ...
        GlobalCtx {
            ...
            extra_network_identities: Mutex::new(vec![]),
    ...
    pub fn is_valid_network_identity(&self, peer_identity: &NetworkIdentity) -> bool {
        if &self.get_network_identity() == peer_identity {
            return true;
        }
        let extra = self.extra_network_identities.lock().unwrap();
        extra.contains(peer_identity)
    }

    pub fn set_extra_network_identities(&self, identities: Vec<NetworkIdentity>) {
        *self.extra_network_identities.lock().unwrap() = identities;
    }

    pub fn set_extra_secrets_file(&self, path: String) {
        *self.extra_secrets_file.lock().unwrap() = Some(path);
    }

    pub fn load_extra_identities_from_json(&self) {
        let path = self.extra_secrets_file.lock().unwrap().clone();
        let path = match path {
            Some(p) => p,
            None => return,
        };
        ...
        let secrets: Vec<serde_json::Value> = match serde_json::from_str(&content) {
            Ok(s) => s,
            Err(_) => return,
        };
        let primary_name = self.get_network_identity().network_name.clone();
        let identities: Vec<NetworkIdentity> = secrets
            .iter()
            .filter_map(|s| {
                let secret = s.get("secret")?.as_str()?;
                Some(NetworkIdentity::new(primary_name.clone(), secret.to_string()))
            })
            .collect();
        if !identities.is_empty() {
            self.set_extra_network_identities(identities);
        }
    }

    pub fn set_ip_whitelist_file(&self, path: String) {
        *self.ip_whitelist_file.lock().unwrap() = Some(path);
    }

    pub fn set_ip_hostname_auto_file(&self, path: String) {
    ...
    pub fn bind_hostname(&self, ip: &str, hostname: &str) {
        ...
        let path = self.ip_hostname_auto_file.lock().unwrap().clone();
        let path = match path {
            Some(p) => p,
            None => return,
        };
        let mut bindings: Vec<serde_json::Value> = std::fs::read_to_string(&path)
            .ok()
            .and_then(|c| serde_json::from_str(&c).ok())
            .unwrap_or_default();
        let exists = bindings.iter().any(|v| v.get("ip").and_then(|s| s.as_str()) == Some(ip));
        if !exists {
            bindings.push(serde_json::json!({ "ip": ip, "hostname": hostname }));
            if let Ok(content) = serde_json::to_string(&bindings) {
                let _ = std::fs::write(&path, content);
            }
        }
    }

    pub fn load_ip_whitelist(&self) -> Vec<WhitelistEntry> {
        let path = self.ip_whitelist_file.lock().unwrap().clone();
        let path = match path {
            Some(p) => p,
            None => return vec![],
        };
