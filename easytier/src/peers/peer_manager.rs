...
impl PeerManager {
    ...
    async fn add_new_peer_conn(&self, peer_conn: PeerConn) -> Result<(), Error> {
        if !self.global_ctx.is_valid_network_identity(&peer_conn.get_network_identity()) {
            return Err(Error::SecretKeyError(
                "network identity not match".to_string(),
            ));
        }
        let peer_id = peer_conn.get_peer_id();
        if self.global_ctx.is_peer_blocked(peer_id) {
            return Err(Error::SecretKeyError(
                "peer is blocked by IP whitelist".to_string(),
            ));
        }
