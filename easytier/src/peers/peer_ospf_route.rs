...
impl Route for PeerRoute {
    ...
    async fn list_routes(&self) -> Vec<crate::proto::cli::Route> {
        ...
        for item in route_table.peer_infos.iter() {
            ...
            let Some(next_hop_peer) = route_table.get_next_hop(*item.key()) else {
                tracing::debug!("list_routes: skipping peer {} ({}): no next_hop, ipv4={:?}", *item.key(), item.hostname.as_deref().unwrap_or("?"), item.ipv4_addr);
                continue;
            };
        ...
        let total_peers = route_table.peer_infos.len();
        tracing::debug!("list_routes: total_peers_in_table={}, returned_routes={}", total_peers, routes.len());
        routes
    }
    ...
    async fn get_peer_info(&self, peer_id: PeerId) -> Option<RoutePeerInfo> {
        let info = self.service_impl
            .route_table
            .peer_infos
            .get(&peer_id)
            .map(|x| x.clone());
        if info.is_none() {
            tracing::debug!("get_peer_info: peer {} not found in route_table.peer_infos ({} entries)", peer_id, self.service_impl.route_table.peer_infos.len());
        }
        info
    }
