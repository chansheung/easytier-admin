...
export class ApiClient {
    ...
    public async list_ip_whitelist(): Promise<Array<any>> {
        const response = await this.client.get<any, Array<any>>('/admin/ipwhitelist');
        return response;
    }

    public async create_ip_whitelist_entry(ip: string, comment: string): Promise<undefined> {
        await this.client.post<any>('/admin/ipwhitelist/create', { ip, comment });
    }

    public async delete_ip_whitelist_entry(id: number): Promise<undefined> {
        await this.client.post<any>('/admin/ipwhitelist/delete', { id });
    }

    public async unbind_ip_whitelist_hostname(id: number): Promise<undefined> {
    ...
    public async update_device_instance_state(machine_id: string, inst_id: string, disabled: boolean): Promise<undefined> {
        await this.client.put<any>(`/machines/${machine_id}/networks/${inst_id}`, { disabled });
    }

    public async get_network_info(machine_id: string, inst_id: string): Promise<Record<string, any>> {
