    ...
    async fn handle_heartbeat(
        ...
        // Check if machine is blocked
        {
            ...
            let blocked = crate::db::entity::blocked_devices::Entity::find()
                .filter(crate::db::entity::blocked_devices::Column::MachineId.eq(machine_id.to_string()))
                .one(storage.db().orm_db())
                .await
                .map_err(|e| anyhow::anyhow!("DB error checking block: {:?}", e))?;
            if let Some(b) = blocked {
                tracing::warn!(
                    "Blocked device {} attempted heartbeat: {}",
                    machine_id, b.reason
                );
                return Err(anyhow::anyhow!(
                    "Device is blocked by admin. Reason: {}",
                    b.reason
                )
                .into());
            }
