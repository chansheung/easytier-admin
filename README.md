...
- **流量配额**：按虚拟 IP 限制周期流量（小时/日/周/月），超额自动封禁，复用白名单机制全网隔离
...
流量配额功能用于限制特定虚拟 IP 在指定时间周期内的流量使用量，超额后自动从白名单移除并全网隔离。

#### 使用方式

登录 Admin UI 后，点击侧边栏「📊 流量配额」进入流量配额页面。该页面包含两部分：

- **配额管理**：点击「＋ 添加配额」，输入虚拟 IP、选择周期类型（每小时/每天/每周/每月）和流量限额（支持 MB/GB/TB 单位），即可为该 IP 创建配额规则。
- **流量用量**：实时展示各 IP 在当前周期内的已用流量、时间窗口和更新时间。

#### 工作原理

1. **流量采集**：admin 和 agent 节点的 `easytier-core` 每 30 秒通过 `ADMIN_API_URL` 上报各自直连 peer 的流量统计（rx/tx 字节），附带 HMAC-SHA256 签名（`TRAFFIC_REPORT_SECRET`）防伪造
2. **增量计算**：admin 后端按 `(上报节点, 客户端 IP)` 维度维护内存快照，计算增量 Δ = max(0, 当前值 - 上次值)，宁漏不溢（连接重建导致计数器回退时 Δ=0，避免误封）
3. **配额检查**：每次上报后检查该 IP 各启用周期的累计流量是否超过限额
4. **超额封禁**：超过限额 → 从 `ip_whitelist` 删除该 IP → 复用现有白名单封禁链路（entrypoint 导出 → agent sync → 双端 block_peer → OSPF 路由扩散全网隔离）
5. **解封**：管理员在「IP 白名单」页面手动将该 IP 重新添加到白名单即可

#### 计量说明

| 项目 | 说明 |
|------|------|
| 计量粒度 | 按虚拟 IP，不区分用户/设备 |
| 统计范围 | 仅统计经过上报节点（admin + agent）直连连接的流量；客户端间直连流量不计 |
| 精度 | 30 秒采样粒度，连接重建时丢失单个周期（30s）的流量，对 GB 级阈值影响可忽略 |
| 封禁延迟 | 最长约 90 秒（上报 30s + 白名单导出 30s + core 检查 30s） |
| 冷却期 | admin 后端启动后 90 秒内只建快照不封禁，防止重启误封 |

#### 周期类型

| 周期 | period_key 格式 | 重置时机 |
|------|----------------|----------|
| 每小时 | `2026-06-23T14` | 每整点 |
| 每天 | `2026-06-23` | 每日 0 点 |
| 每周 | `2026-W26` | 每周一（ISO 周） |
| 每月 | `2026-06` | 每月 1 号 |

一个 IP 可同时配置多个周期上限（如每天 10GB + 每月 200GB），任一周期超额即封禁。

> **注意**：超额封禁后不会自动恢复。即使进入新周期（流量计数归零），被封 IP 仍不在白名单中，需管理员手动添加回白名单方可解封。
...
| `TRAFFIC_REPORT_SECRET` | 默认同 `ET_ADMIN_SECRET` | 流量上报 HMAC 签名密钥，agent 端需设为相同值 |
...
| `ADMIN_API_URL` | （需配置） | admin 管理面板地址，如 `http://10.0.10.1:11211`，用于上报 peer 流量数据 |
| `TRAFFIC_REPORT_SECRET` | `easytier-admin-secret-change-me` | 流量上报 HMAC 签名密钥，需与 admin 端一致 |
